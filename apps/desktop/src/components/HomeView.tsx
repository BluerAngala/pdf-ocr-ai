...
const MODULES: { key: ModuleType; title: string; desc: string; color: string; iconBg: string; hoverBorder: string; hoverIconBg: string; arrowColor: string; iconPath: string }[] = [
  {
    key: 'non-litigation',
    title: '非诉审查',
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 gap-8">
        ...
        <div className="grid grid-cols-3 gap-6 w-full max-w-3xl">
          {MODULES.map(m => (
            ...
            >
              <div className={`w-10 h-10 ${m.iconBg} rounded-lg flex items-center justify-center mb-4 transition-colors`}>
                <svg className={`w-5 h-5 ${ICON_COLORS[m.color]}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={m.iconPath} />
                </svg>
              </div>
              ...
              <div className={`mt-3 flex items-center gap-1 text-xs font-medium ${m.arrowColor} opacity-0 group-hover:opacity-100 transition-opacity`}>
                <span>进入模块</span>
