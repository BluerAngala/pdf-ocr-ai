    ...
    <div className="flex-1 flex flex-col min-h-0">
      <div className="h-10 bg-white border-b border-slate-200 flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="inline-flex items-center justify-center w-7 h-7 rounded-md text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onPreset} className="px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100 transition-colors cursor-pointer">快速预设</button>
          <button onClick={onRun} disabled={running} className="px-4 py-1.5 text-xs font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 active:scale-[0.97] transition-all shadow-sm cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">开始处理</button>
        </div>
      </div>
