...
公积金业务辅助工具，RapidOCR + pdfplumber 的 PDF/图片 OCR 识别。
...
python apps/server/scripts/run_enforcement_flow.py                  # 强制执行组
python apps/server/scripts/run_enforcement_flow.py --use-ocr        # 强制执行组（OCR 模式）

pytest apps/server/tests/non-litigation/                             # 非诉组测试
pytest apps/server/tests/non-litigation/test_non_litigation_export.py      # 单测试文件
...
两条业务主线共享 OCR 基础设施，通过 `server.py` 的 JSON-RPC 暴露给 Tauri 前端（stdin/stdout 通信，stderr 推进度通知）。
...
└── server/                          # Python 后端
    ...
    │   │
    │   ├── company_query.py               # unicloud API 查询公司信息（旧 Coze 已弃用）
...
- **enforcement_extractor** 先 `remove_cjk_spacing()` 再做正则匹配，调试时注意处理后的文本
- Rust 侧将 Python cwd 设为项目根目录
- 输出到 `output/` 和 `temp/`，已被 git 忽略；断点续跑，已有缓存自动跳过
- `样本材料/` 含真实样本 PDF 和标准输出，测试依赖这些文件
- **强制执行组无独立测试目录**，改动后手动跑 `run_enforcement_flow.py` 验证
- **company_query** 已从 Coze 迁移到 unicloud，旧 Coze 配置仍在 config.yaml 但已弃用
...
8. `enforcement.excel_parsing` / `enforcement.ocr_corrections` — 强制执行组对应配置
